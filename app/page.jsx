...
// Builds headers with the current Supabase JWT — used by every API call
...
function ProfilePage({ user, profile, setProfile, setPage, setOpportunities, setLoadingOps, initialUsage }) {
  ...
  function refreshUsage() {
    if (!user?.id) return;
    authHeaders().then(h => fetch('/api/usage', { headers: h })).then(r => r.json()).then(d => { if (!d.error) setUsage(d); }).catch(() => {});
  }
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/profile', { method:'POST', headers: await authHeaders(), body: JSON.stringify({ profile: form }) });
      const data = await res.json();
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      // Persist to DB
      authHeaders().then(h => fetch('/api/opportunities', {
        method: 'POST', headers: h,
        body: JSON.stringify({ opportunities: ops }),
      })).catch(() => {});
